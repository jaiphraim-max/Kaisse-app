...
const state = {
  ...
  zClosures: [],
  settings: {
    shopName: 'Mon Restobar', currency: 'Ar', cashierPin: '1234', adminPin: '9999', logoDataUrl: '',
    ...
    kitchenPrinterEnabled: false, kitchenPrinterType: 'none', kitchenPrinterName: '', kitchenPrinterWidth: '80', kitchenPrinterBridgeUrl: '',
    receiptPrinterEnabled: false, receiptPrinterType: 'none', receiptPrinterName: '', receiptPrinterWidth: '80', receiptPrinterBridgeUrl: '',
    invoicePrinterEnabled: false, invoicePrinterType: 'none', invoicePrinterName: '', invoicePrinterWidth: 'A4', invoicePrinterBridgeUrl: '',
  },
  activeCategory: 'all',
...
async function loadAll() {
  ...
  const logoDataUrl = await Store.get('settings', 'logoDataUrl');
  ...
  const kitchenPrinterBridgeUrl = await Store.get('settings', 'kitchenPrinterBridgeUrl');
  ...
  const receiptPrinterBridgeUrl = await Store.get('settings', 'receiptPrinterBridgeUrl');
  ...
  const invoicePrinterBridgeUrl = await Store.get('settings', 'invoicePrinterBridgeUrl');
  if (shopName) state.settings.shopName = shopName.value;
  if (logoDataUrl) state.settings.logoDataUrl = logoDataUrl.value;
  ...
  if (kitchenPrinterBridgeUrl) state.settings.kitchenPrinterBridgeUrl = kitchenPrinterBridgeUrl.value;
  ...
  if (receiptPrinterBridgeUrl) state.settings.receiptPrinterBridgeUrl = receiptPrinterBridgeUrl.value;
  ...
  if (invoicePrinterBridgeUrl) state.settings.invoicePrinterBridgeUrl = invoicePrinterBridgeUrl.value;
  applyTheme();
}
...
function formatOptions(options) {
  if (!options) return '';
  const parts = [];
  if (options.cooking) parts.push(options.cooking);
  if (options.sauce) parts.push('Sauce ' + options.sauce);
  let result = parts.join(' ');
  if (options.side) {
    const sides = options.side.split(' + ').join(', ');
    result = result ? result + '/' + sides : sides;
  }
  return result;
}
...
/* ===========================================================
   IMPRESSION THERMIQUE — pont HTTP vers imprimante réseau
   Un navigateur ne peut pas parler directement au port 9100 (TCP brut),
   donc Kaisse envoie les données à un petit pont HTTP (voir guide fourni)
   qui les retransmet à l'imprimante.
=========================================================== */
...
function logoToEscPosBytes(dataUrl) {
    ...
    try {
      ...
      img.onload = () => {
        try {
          ...
          // Largeur cible en pixels : multiple de 8. Les intents Android ont une limite
          // pratique bien plus stricte qu'attendu (souvent ~40-80 Ko selon le fabricant/OS) ;
          // un logo trop grand tronque silencieusement le flux et corrompt tout le ticket,
          // désynchronisant l'imprimante. On garde donc le logo volontairement petit.
          const maxWidth = 120;
          const maxHeight = 120;
          ...
          w = w - (w % 8); // GS v 0 exige une largeur multiple de 8
          if (w < 8 || h < 1) { resolve(null); return; }
          ...
          try {
            ...
            resolve(null); return; // ex: image cross-origin non lisible
          }
          // Vérification que le canvas a bien produit la quantité de pixels attendue
          if (!imageData || imageData.length !== w * h * 4) { resolve(null); return; }

          const bytesPerRow = w / 8;
          const expectedLength = bytesPerRow * h;
          const bitmap = new Uint8Array(expectedLength);
          ...
          for (let y = 0; y < h; y++) {
            for (let x = 0; x < w; x++) {
              ...
              const isDark = a > 128 && luminance < 160; // seuil simple noir/blanc
              if (isDark) {
          ...
          if (bytesPerRow > 0xFFFF || h > 0xFFFF) { resolve(null); return; }
          // Limite dure sur la taille du bitmap lui-même, volontairement basse pour
          // rester sous la vraie limite pratique des intents Android une fois combiné
          // au texte du ticket et encodé en base64 (+33% de volume).
          if (bitmap.length > 4000) { resolve(null); return; }
          ...
          // Vérification finale absolue : la taille du buffer final doit être exactement
          // header + bitmap, rien de plus, rien de moins.
          if (result.length !== header.length + bitmap.length) { resolve(null); return; }
...
function toPrinterSafeText(str) {
  // Les imprimantes thermiques ESC/POS n'affichent correctement que la table CP437 par défaut.
  // On translittère les accents et on remplace les espaces spéciaux (insécables) par des espaces normaux.
  const map = {
    'à':'a','â':'a','ä':'a','á':'a','ã':'a',
    'é':'e','è':'e','ê':'e','ë':'e',
    'î':'i','ï':'i','í':'i','ì':'i',
  ...
  return str.replace(/[^\x00-\x7F]/g, (ch) => map[ch] !== undefined ? map[ch] : '?');
}

function getReceiptCharWidth() {
  // Nombre de caractères par ligne en police A, selon la largeur de papier configurée
  const width = state.settings.receiptPrinterWidth;
  if (width === '80') return 48;
  if (width === '58') return 32;
  if (width === '50') return 32;
  return 48; // 80mm par défaut
}
...
function buildReceiptEscPos(sale) {
  ...
  const line = (l, r) => {
    const pad = Math.max(1, charWidth - l.length - r.length);
    return l + ' '.repeat(pad) + r + '\n';
  };
  let out = ESCPOS.init;
  out += ESCPOS.alignCenter + ESCPOS.boldOn + s.shopName + '\n' + ESCPOS.boldOff;
  if (s.address) out += s.address + '\n';
  if (s.phone) out += 'Tel: ' + s.phone + '\n';
  if (s.nif) out += 'NIF: ' + s.nif + '\n';
  out += sep;
  out += ESCPOS.alignLeft;
  out += line('Date', new Date(sale.createdAt).toLocaleString('fr-FR'));
  out += line('Ticket', '#' + sale.id.slice(-6).toUpperCase());
  out += sep;
  sale.lines.forEach(l => {
    const nameMax = charWidth - 6;
    out += line(`${l.qty}x ${l.name}`.slice(0, nameMax), fmt(l.price * l.qty));
    if (l.qty > 1) out += '  PU: ' + fmt(l.price) + '\n';
    const opts = formatOptions(l.options);
    if (opts) out += '  ' + opts + '\n';
  });
  out += sep;
  out += ESCPOS.boldOn + line('TOTAL', fmt(sale.total)) + ESCPOS.boldOff;
  out += line('Paiement', paymentLabel(sale.paymentMethod));
  if (sale.cashGiven !== null) {
    out += line('Recu', fmt(sale.cashGiven));
    out += line('Monnaie', fmt(sale.changeDue));
  }
  out += sep;
  out += ESCPOS.alignCenter + s.thankYouMessage + '\n';
  out += ESCPOS.feed(3) + ESCPOS.cut;
  return toPrinterSafeText(out);
}

function textToLatin1Bytes(str) {
  // str est déjà nettoyé par toPrinterSafeText (ASCII pur), donc chaque caractère
  // correspond exactement à un octet — pas besoin d'UTF-8 ici, qui casserait tout
  // si un caractère résiduel non-ASCII passait au travers.
...
function repeatEscPosContent(byteArray, copies) {
  if (copies <= 1) return byteArray;
  const parts = [];
...
function printViaRawBTBytes(byteArray) {
  // RawBT reçoit les données ESC/POS encodées en base64 via son intent Android officiel.
  // Format exact documenté par RawBT (rawbt.ru/start.html) : le contenu "rawbt:base64,..."
  // doit être encodé avec encodeURI() avant d'être placé après "intent:".

  // Sécurité : un intent Android trop volumineux peut être tronqué silencieusement
  // par le système, ce qui désynchronise l'imprimante pour le reste du flux — d'où
  // une limite volontairement basse et un logo volontairement léger.
  if (byteArray.length > 8000) {
    toast('Ticket trop volumineux pour être envoyé — réduisez la taille du logo');
    return false;
  }
  ...
  const rawbtPayload = 'rawbt:base64,' + base64Content;
  const textEncoded = encodeURI(rawbtPayload);
  const intentSuffix = '#Intent;scheme=rawbt;package=ru.a402d.rawbtprinter;end;';

  window.location.href = 'intent:' + textEncoded + intentSuffix;
  return true;
}
...
async function buildTicketWithOptionalLogo(textContent) {
  const parts = [];
  parts.push(new Uint8Array([0x1B, 0x40])); // ESC @ — init
  parts.push(new Uint8Array([0x1B, 0x61, 0x01])); // centré, pour le logo

  if (state.settings.logoDataUrl) {
    const logoBytes = await logoToEscPosBytes(state.settings.logoDataUrl);
    if (logoBytes) {
      const withLogo = concatUint8Arrays([...parts, logoBytes, textToLatin1Bytes('\n'), textToLatin1Bytes(textContent)]);
      // Si le ticket complet (logo + texte) risque de dépasser la limite sûre de
      // l'intent Android, on repart sans le logo plutôt que de tout tronquer.
      if (withLogo.length <= 8000) {
        return withLogo;
      }
      toast('Logo retiré (ticket trop volumineux) — impression sans logo');
    } else {
      toast('Logo ignoré (image invalide) — ticket imprimé sans logo');
    }
  }

  parts.push(textToLatin1Bytes(textContent));
  return concatUint8Arrays(parts);
}

async function printTextWithLogoToPrinter(textContent, buttonId, successLabel, copies) {
  copies = copies || 1;
  const printBtn = document.getElementById(buttonId);
  if (printBtn) { printBtn.disabled = true; printBtn.textContent = 'Envoi...'; }
  try {
    const fullTicket = await buildTicketWithOptionalLogo(textContent);

    for (let i = 0; i < copies; i++) {
      if (i > 0) {
        await new Promise(resolve => setTimeout(resolve, 1200));
      }
      printViaRawBTBytes(fullTicket);
    }
    toast(copies > 1 ? copies + ' copies envoyées à RawBT' : (successLabel || 'Envoyé à RawBT'));
  } catch (e) {
    toast('Impossible d\'ouvrir RawBT — vérifiez qu\'il est bien installé');
  } finally {
    if (printBtn) { printBtn.disabled = false; printBtn.textContent = 'Imprimer'; }
  }
}

function buildXReportEscPos(summary) {
  const s = state.settings;
  const width = getReceiptCharWidth();
  const line = (l, r) => {
  ...
  out += '-'.repeat(width) + '\n';
  out += ESCPOS.alignLeft;
  out += line('Genere le', new Date().toLocaleString('fr-FR'));
  out += line('Nombre de tickets', String(summary.count));
  out += '-'.repeat(width) + '\n';
  Object.entries(summary.byMethod).forEach(([method, amt]) => {
    out += line(paymentLabel(method), fmt(amt));
  });
  out += '-'.repeat(width) + '\n';
  out += ESCPOS.boldOn + line('TOTAL', fmt(summary.total)) + ESCPOS.boldOff;
  out += ESCPOS.feed(3) + ESCPOS.cut;
  return toPrinterSafeText(out);
}

function printXReportToPrinter(summary) {
  printTextWithLogoToPrinter(buildXReportEscPos(summary), 'modalPrintX', 'Rapport X envoyé à RawBT');
}

function buildZReceiptEscPos(closure) {
  const s = state.settings;
  const width = getReceiptCharWidth();
  const line = (l, r) => {
    const pad = Math.max(1, width - l.length - r.length);
  ...
  out += ESCPOS.alignCenter + ESCPOS.boldOn + s.shopName + '\n' + ESCPOS.boldOff;
  if (s.address) out += s.address + '\n';
  if (s.nif) out += 'NIF: ' + s.nif + (s.stat ? ' STAT: ' + s.stat : '') + '\n';
  out += 'CLOTURE Z #' + closure.number + '\n';
  out += '-'.repeat(width) + '\n';
  out += ESCPOS.alignLeft;
  out += line('Date de cloture', new Date(closure.closedAt).toLocaleString('fr-FR'));
  out += line('Nombre de tickets', String(closure.ticketCount));
  if (closure.avgTicket !== undefined) {
    out += line('Panier moyen', fmt(closure.avgTicket));
  }
  out += '-'.repeat(width) + '\n';
  Object.entries(closure.byMethod).forEach(([method, amt]) => {
    out += line(paymentLabel(method), fmt(amt));
  });
  if (closure.byCategory && Object.keys(closure.byCategory).length > 0) {
    out += '-'.repeat(width) + '\n';
    out += 'VENTES PAR CATEGORIE\n';
    Object.entries(closure.byCategory).forEach(([cat, data]) => {
      const amt = typeof data === 'object' ? data.amount : data;
      const qty = typeof data === 'object' ? data.qty : null;
      const label = qty !== null ? cat + ' (' + qty + ')' : cat;
      out += line(label, fmt(amt));
    });
  }
  out += '-'.repeat(width) + '\n';
  out += ESCPOS.boldOn + line('TOTAL CLOTURE', fmt(closure.total)) + ESCPOS.boldOff;
  out += ESCPOS.feed(3) + ESCPOS.cut;
  return toPrinterSafeText(out);
}

function printZReceiptToPrinter(closure, copies) {
  printTextWithLogoToPrinter(buildZReceiptEscPos(closure), 'modalPrintZ', 'Clôture Z envoyée à RawBT', copies);
}

async function printReceiptToBridge(sale, copies) {
  copies = copies || 1;
  const printBtn = document.getElementById('modalPrintReceipt');
  if (printBtn) { printBtn.disabled = true; printBtn.textContent = 'Envoi...'; }
  try {
    const textContent = buildReceiptEscPos(sale);
    const fullTicket = await buildTicketWithOptionalLogo(textContent);

    for (let i = 0; i < copies; i++) {
      if (i > 0) {
        // Petite pause entre deux envois pour laisser RawBT/l'imprimante traiter le précédent
        await new Promise(resolve => setTimeout(resolve, 1200));
      }
      printViaRawBTBytes(fullTicket);
    }
    toast(copies > 1 ? copies + ' copies envoyées à RawBT' : 'Ticket envoyé à RawBT');
  } catch (e) {
    toast('Impossible d\'ouvrir RawBT — vérifiez qu\'il est bien installé');
  } finally {
    if (printBtn) { printBtn.disabled = false; printBtn.textContent = 'Imprimer'; }
...
function showFinalReceipt(sale) {
  ...
  showModal(`
    ...
    <div class="receipt-box">
      <div style="text-align:center; margin-bottom:8px;">
        ${s.logoDataUrl ? `<img src="${s.logoDataUrl}" style="max-width:100px; max-height:100px; margin-bottom:6px;" alt="Logo">` : ''}
      ...
      ${sale.lines.map(l => `
        <div class="r-line"><span>${l.qty}× ${escapeHtml(l.name)}</span><span>${fmt(l.price * l.qty)}</span></div>
        ${l.qty > 1 ? `<div class="r-options">PU: ${fmt(l.price)}</div>` : ''}
        ${formatOptions(l.options) ? `<div class="r-options">${escapeHtml(formatOptions(l.options))}</div>` : ''}
      `).join('')}
      <hr>
    ...
    ${s.receiptPrinterEnabled ? `
    <div class="form-group" style="margin-top:12px;">
      <label>Nombre de copies à imprimer</label>
      <select id="receiptCopiesSelect">
    ...
    ` : ''}
    <div class="modal-actions">
      ${s.receiptPrinterEnabled ? `<button class="btn btn-secondary" id="modalPrintReceipt">Imprimer</button>` : ''}
  ...
  if (printBtn) printBtn.addEventListener('click', () => {
    const copiesSelect = document.getElementById('receiptCopiesSelect');
    const copies = copiesSelect ? parseInt(copiesSelect.value) : 1;
    printReceiptToBridge(sale, copies);
  });
  ...
  showModal(`
    ...
    <div class="receipt-box">
      <div style="text-align:center; margin-bottom:10px;">
        ${s.logoDataUrl ? `<img src="${s.logoDataUrl}" style="max-width:110px; max-height:110px; margin-bottom:6px;" alt="Logo">` : ''}
      ...
      ${sale.lines.map(l => `
        <div class="r-line"><span>${l.qty}× ${escapeHtml(l.name)}</span><span>${fmt(l.price * l.qty)}</span></div>
        <div class="r-options">PU: ${fmt(l.price)}</div>
        ${formatOptions(l.options) ? `<div class="r-options">${escapeHtml(formatOptions(l.options))}</div>` : ''}
      `).join('')}
      <hr>
...
function buildReportSummary(sales) {
  ...
  sales.forEach(s => {
    s.lines.forEach(l => {
      ...
      if (!byCategory[catName]) byCategory[catName] = { amount: 0, qty: 0 };
      byCategory[catName].amount += l.price * l.qty;
      byCategory[catName].qty += l.qty;
    });
  });
  const count = sales.length;
  const avgTicket = count > 0 ? total / count : 0;
  return { total, count, byMethod, byCategory, avgTicket };
}
...
function showXReport() {
  ...
  showModal(`
    ...
    <div class="receipt-box">
      ...
      <div class="r-line"><span>Panier moyen</span><span>${fmt(summary.avgTicket)}</span></div>
      <hr>
      ${Object.entries(summary.byMethod).map(([method, amt]) => `
        <div class="r-line"><span>${paymentLabel(method)}</span><span>${fmt(amt)}</span></div>
      `).join('')}
      <hr>
      <div style="font-weight:700; margin-bottom:4px;">Ventes par catégorie</div>
      ${Object.entries(summary.byCategory).map(([cat, amt]) => `
        <div class="r-line"><span>${escapeHtml(cat)}</span><span>${fmt(amt)}</span></div>
      `).join('') || '<div class="r-line"><span>Aucune vente</span></div>'}
      <hr>
      <div class="r-line"><strong>Total</strong><strong>${fmt(summary.total)}</strong></div>
    </div>
    <div class="modal-actions">
      ${state.settings.receiptPrinterEnabled ? `<button class="btn btn-secondary" id="modalPrintX">Imprimer</button>` : ''}
      <button class="btn btn-primary" id="modalCloseX">Fermer</button>
    </div>
  `);
  const printXBtn = document.getElementById('modalPrintX');
  if (printXBtn) printXBtn.addEventListener('click', () => printXReportToPrinter(summary));
...
function showZReceipt(closure) {
  ...
  showModal(`
    ...
    <div class="receipt-box">
      ...
      ${closure.avgTicket !== undefined ? `<div class="r-line"><span>Panier moyen</span><span>${fmt(closure.avgTicket)}</span></div>` : ''}
      <hr>
      ${Object.entries(closure.byMethod).map(([method, amt]) => `
        <div class="r-line"><span>${paymentLabel(method)}</span><span>${fmt(amt)}</span></div>
      `).join('')}
      ${closure.byCategory && Object.keys(closure.byCategory).length > 0 ? `
        <hr>
        <div style="font-weight:700; margin-bottom:4px;">Ventes par catégorie</div>
        ${Object.entries(closure.byCategory).map(([cat, amt]) => `
          <div class="r-line"><span>${escapeHtml(cat)}</span><span>${fmt(amt)}</span></div>
        `).join('')}
      ` : ''}
      <hr>
    ...
    ` : ''}
    <div class="modal-actions">
      ${s.receiptPrinterEnabled ? `<button class="btn btn-secondary" id="modalPrintZ">Imprimer</button>` : ''}
      <button class="btn btn-primary" id="modalCloseZ">Fermer</button>
    </div>
  `);
  const printZBtn = document.getElementById('modalPrintZ');
  if (printZBtn) printZBtn.addEventListener('click', () => {
    const copiesSelect = document.getElementById('zCopiesSelect');
    const copies = copiesSelect ? parseInt(copiesSelect.value) : 1;
    printZReceiptToPrinter(closure, copies);
  });
...
function handleLogoFileSelected(e) {
  ...
  reader.onload = (ev) => {
    const img = new Image();
    img.onload = async () => {
      // Redimensionne pour rester léger en stockage local et adapté à un ticket thermique
      const maxWidth = 300;
      ...
      await Store.put('settings', { key: 'logoDataUrl', value: dataUrl });
      updateLogoPreview();
...
function renderParametres() {
  ...
  document.getElementById('btnUploadLogo').addEventListener('click', () => {
    document.getElementById('settingLogoFile').click();
  });
  document.getElementById('settingLogoFile').addEventListener('change', handleLogoFileSelected);
  document.getElementById('btnRemoveLogo').addEventListener('click', async () => {
    state.settings.logoDataUrl = '';
    await Store.put('settings', { key: 'logoDataUrl', value: '' });
    updateLogoPreview();
    toast('Logo retiré');
  });
...
function printerBlockHtml(def, s) {
  ...
  return `
    <div class="option-group">
      ...
      <div class="form-group printer-bridge-field" data-printer-key="${k}" style="${s[k + 'PrinterType'] === 'wifi' ? '' : 'display:none;'}">
        <label>Adresse du pont d'impression</label>
        <input type="text" id="${k}PrinterBridgeUrl" value="${escapeHtml(s[k + 'PrinterBridgeUrl'] || '')}" placeholder="Ex: 192.168.43.50:8765">
        <p class="settings-hint" style="margin-top:4px;">Adresse IP et port du téléphone qui fait tourner le pont d'impression (voir le guide fourni).</p>
      </div>
    </div>
  `;
}
...
function renderPrinterSettingsContent() {
  ...
  showModal(`
    ...
    <p class="settings-hint">Pour l'imprimante caisse, Kaisse utilise l'application <strong>RawBT</strong> (à installer séparément) qui gère la connexion réelle à l'imprimante (USB, Bluetooth ou Wi-Fi) — activez simplement l'imprimante caisse ci-dessus, aucune autre configuration n'est nécessaire ici. La facture formelle se génère depuis l'écran d'encaissement pour les clients qui en font la demande.</p>
  ...
  PRINTER_DEFS.forEach(def => {
    const typeSelect = document.getElementById(def.key + 'PrinterType');
    typeSelect.addEventListener('change', () => {
      const bridgeField = document.querySelector(`.printer-bridge-field[data-printer-key="${def.key}"]`);
      bridgeField.style.display = typeSelect.value === 'wifi' ? 'block' : 'none';
    });
  });
  ...
  document.getElementById('modalSavePrinters').addEventListener('click', async () => {
    for (const def of PRINTER_DEFS) {
      ...
      state.settings[k + 'PrinterBridgeUrl'] = document.getElementById(k + 'PrinterBridgeUrl').value.trim();
      ...
      await Store.put('settings', { key: k + 'PrinterBridgeUrl', value: state.settings[k + 'PrinterBridgeUrl'] });
    }
